...
public final class TeamHomeValue extends TeamValue<Location, IdentifiableLocation> {
    ...
    public IdentifiableLocation parse(Record record) {
        return new IdentifiableLocation(record.get(TeamValueOld.ID), new Location(Bukkit.getWorld(record.get(this.world)), record.get(this.locationX), record.get(this.locationY), record.get(this.locationZ), record.get(this.locationYaw).floatValue(), record.get(this.locationPitch).floatValue()));
    }
}
