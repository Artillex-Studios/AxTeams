...
public final class TeamWarpsValue extends TeamValue<Warp, Warp> {
    ...
    public Warp parse(Record record) {
        return new Warp(record.get(TeamValue.ID), record.get(this.name), new Location(Bukkit.getWorld(record.get(this.world)), record.get(this.locationX), record.get(this.locationY), record.get(this.locationZ), record.get(this.locationYaw).floatValue(), record.get(this.locationPitch).floatValue()), record.get(this.password));
    }
}
