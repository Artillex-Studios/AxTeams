...
public final class FileUtils {
    public static final Path PLUGIN_DIRECTORY = AxTeamsPlugin.instance().getDataFolder().toPath();
    private static final Logger log = LoggerFactory.getLogger(FileUtils.class);

    public static void copyFromResource(@NotNull String path) {
        try (ZipFile zip = new ZipFile(Paths.get(AxTeamsPlugin.instance().getClass().getProtectionDomain().getCodeSource().getLocation().toURI()).toFile())) {
            for (Iterator<? extends ZipEntry> it = zip.entries().asIterator(); it.hasNext(); ) {
                ...
                InputStream resource = AxTeamsPlugin.instance().getResource(entry.getName());
                if (resource == null) {
